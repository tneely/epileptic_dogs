...
function feats = features(X,freq) 
    ...
    for i = 1:segs
        ...
        feats(i,1*chnls+1:3*chnls) = signal_power(freq_x, theta, f);
        feats(i,3*chnls+1:4*chnls) = signal_power(freq_x, delta, f);
        feats(i,4*chnls+1:5*chnls) = signal_power(freq_x, alpha, f);
        feats(i,5*chnls+1:6*chnls) = signal_power(freq_x, beta, f);
        feats(i,6*chnls+1:7*chnls) = signal_power(freq_x, gamma, f);
